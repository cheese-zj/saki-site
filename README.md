...
The silent homepage mosaic loops unless reduced motion is requested, in which case its poster remains visible until the visitor chooses to play. A visible control pauses or resumes it. Every other video requires user playback. Starting a video pauses the others; leaving the tab pauses playback. The desktop hero expands into a full-height mosaic as the visitor scrolls, with a single title crossfade. Small screens, reduced-motion settings and disabled JavaScript retain normal document flow. Reduced motion also disables the opening text entrance. No analytics, external scripts, third-party embeds or runtime services.
