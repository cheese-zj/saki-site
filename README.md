...
The three method excerpts use exact ranges of that same video: interaction 21–33 s; reuse 33–42 s; assembly 50–65 s. Their posters are sampled at 27, 40 and 62 seconds; the film poster is at 99 seconds. The five-second homepage mosaic is rendered from the original `src/IntroMosaic.tsx` composition, retaining all 20 cells and their source timing but removing the title, desaturation, dimming and camera drift. Its poster is frame 30. Page text uses system Helvetica Neue, with Helvetica and Arial fallbacks, in two responsive sizes: 20 px body and 40–80 px headings on desktop, 18 px body and 36–44 px headings on mobile. Regular-weight headings and slightly tightened heading tracking follow the Sunday reference. Typography burnt into the original video is unchanged. Prepared target paths are explanatory visualisations, not measured tracking results. Assembly footage and point clouds include separate captures; no synchronisation claim is made.

The silent homepage mosaic loops unless reduced motion is requested, in which case its poster remains visible until the visitor chooses to play. There is no overlaid pause button; clicking the mosaic or pressing Space/Enter while it is focused toggles playback. Every other video requires user playback. Demonstrations pause one another while the silent mosaic runs independently. Leaving the tab pauses playback; returning resumes the mosaic unless the visitor paused it or requested reduced motion. The desktop hero expands into a full-height mosaic as the visitor scrolls, with a single title crossfade. Small screens, reduced-motion settings and disabled JavaScript retain normal document flow. Reduced motion also disables the opening text entrance. No analytics, external scripts, third-party embeds or runtime services.
